...
//	}
	...
	/*
	 * @Autowired public SignUpController(InMemoryUserDetailsManager
	 * inMemoryUserDetailsManager) { LOGGER.info("Set user details");
	 * this.appUserDetailsService = inMemoryUserDetailsManager;
	 * 
	 * }
	 */
	
	/*
	 * @GetMapping("/{username}") public boolean
	 * userExists(@PathVariable("username") String username ) {
	 * LOGGER.info("Checking if user exists"); return
	 * appUserDetailsService.userExists(username); }
	 * 
	 * @PostMapping("/adduser") public String add(@RequestBody @Valid User user) {
	 ...
	 * inMemoryUserDetailsManager.createUser(new User(user.getUsername(),
	 * passwordEncoder().encode(user.getPassword()),authorities )); return "added";}
	 * else throw new UserExists(); }
	 * 
	 * private BCryptPasswordEncoder passwordEncoder() { // TODO Auto-generated
	 * method stub return new BCryptPasswordEncoder(); }
	 */
